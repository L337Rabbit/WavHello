...
namespace com.okitoki.wavhello.chunks
{
    public class CueChunk : Chunk
    {
        ...
        public static CueChunk Read(FileStream fs, string chunkID, int chunkSize)
        {
            ...
            chunk.NumCuePoints = BitConverter.ToInt32(BinaryFileUtils.Read(fs, 4));
    ...
    public class CuePoint
    {
        ...
        public static CuePoint Read(FileStream fs)
        {
            ...
            cuePoint.Position = BitConverter.ToInt32(BinaryFileUtils.Read(fs, 4));
            cuePoint.DataChunkID = System.Text.Encoding.UTF8.GetString(BinaryFileUtils.Read(fs, 4));
            cuePoint.ChunkStartOffset = BitConverter.ToInt32(BinaryFileUtils.Read(fs, 4));
            cuePoint.BlockStart = BitConverter.ToInt32(BinaryFileUtils.Read(fs, 4));
            cuePoint.SampleOffset = BitConverter.ToInt32(BinaryFileUtils.Read(fs, 4));
